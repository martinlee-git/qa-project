# OpenAI, Ona 인수
- Source: OpenAI News
- Category: ai_news
- Original URL: https://openai.com/index/openai-to-acquire-ona
- Published At: Thu, 11 Jun 2026 00:00:00 GMT
- Collected At: 2026-06-12T14:00:46.744Z
- Notion Page: https://app.notion.com/p/OpenAI-News-OpenAI-Ona-37cfc93aec578140a23fdd67c3b3b387
- Original Title: OpenAI to acquire Ona
## Summary
OpenAI는 Ona를 인수하여 Codex를 안전하고 지속적인 클라우드 환경으로 확장하여 기업 워크플로 전반에 걸쳐 장기 실행 AI 에이전트를 지원할 계획입니다.

## Original Description

OpenAI plans to acquire Ona to expand Codex with secure, persistent cloud environments, enabling long-running AI agents across enterprise workflows.
## Key Points
- OpenAI는 Ona를 인수하여 Codex를 안전하고 지속적인 클라우드 환경으로 확장하여 기업 워크플로 전반에서 장기 실행 AI 에이전트를 지원할 계획입니다.
## Excerpt